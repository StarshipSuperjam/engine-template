...
One boundary of medium you own: a fact, a bound, or a format that a machine could decide and check, lodged in a standing prose rule instead of code or a checked data file — raise it, because prose there holds only by every reader's compliance while code holds the same way every time. Procedure whose steps carry judgement, and posture like the codes of conduct, are prose's proper home; the test is whether a machine could decide it with no per-case judgement, so weigh that honestly rather than flag judgement prose as a fault.
