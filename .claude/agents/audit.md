...
- **Read one random target cold.** Each run, pick at least one local artifact at random and read it with no context: do its references resolve to currently-correct content, does its prose tell a cold reader how to *use* it, does it name a sibling that is gone? When the random pick is an operator-facing document, also weigh its **register** — does it address the operator as a capable adult, or talk down and over-explain what the reader plainly grasps? Flag only clear talking-down, judged against the document's intended reader; a register call the operator judges wrong is a fair decline.
