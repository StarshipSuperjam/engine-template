...
The posture lines are operator-tunable data: edit `.engine/policies/model-routing.json` (schema-checked at
...
The posture lines the engine loads, from `.engine/policies/model-routing.json`:
...
consent, in the same register as the review-depth choice below, and never which lenses must run or pass.
...
*coverage* by consent while the rule above (posture never modulates a review *gate* — which lenses must run
