...
<!-- posture:qualified -->
```text
This execution environment matches the qualification recorded for this repository.
Operate per the capability tiers in .engine/policies/model-bindings.json.
The running model's identity is not verified by the engine — treat the bindings as intent, not proof of the live model.
```
...
The qualification act is the operator's: `execution_environment.py record <env>` stamps a proposed baseline,
and merging that diff is what qualifies the environment. The engine never qualifies itself. Because the
...
an evolving judgment: the periodic engine audit compares them against the current model landscape and proposes
an update when they no longer fit, so a deployed operator is not left to notice model changes unaided.
`AUDIT_MODEL` (the audit workflow's model knob) is the one place a model is named at invocation and is left to
the operator.
