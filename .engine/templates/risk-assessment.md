...
- **Standard review** — a focused subset of the independent reviews, each at the effort configured for Standard — more independent scrutiny than Quick, less than Thorough runs.
- **Thorough review** — every independent review available, each at the effort configured for Thorough, hardest on the risky parts — the most independent scrutiny, so the most gets caught before it ships.
...
<Include this section ONLY when the change would weaken one of the engine's own guardrails — turning off a check, loosening a block, or editing the protection files. Name, in plain words, WHICH protection weakens and what the AI could then do unwatched. Then say which tier it lands in (eADR-0040): a KILLSWITCH-tier change (repointing where the engine fetches or writes code, demoting a merge-blocking check, deleting a gate, the guard's own machinery) gets the deliberate-confirm note — e.g. "You'll confirm this deliberately at merge — a separate step from the normal merge click." Every OTHER protection-file edit gets the disclosure note instead: "The engine will leave a plain-language notice on the pull request naming the enforcement files this touches. It does not block the merge, needs no label, and is never a reason to change the design — your review at merge is the gate." Describe the actual protection THIS change weakens — do not reuse the examples unless they genuinely fit. Delete this whole section when nothing weakens; strengthening a guardrail is never flagged.>
