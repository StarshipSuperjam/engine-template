...
session, and they're yours: change, add, or remove any of them any time with /engine-setup
($engine-setup in Codex). I didn't put
...
exploration write-gate, and memory capture. Use the Codex CLI /hooks approval browser; do not assume Desktop
exposes a Hooks settings screen. Project trust alone is insufficient. Verify an actual hook event in your
task after approval. The tested Desktop task needed reopening to load its configuration. The VS Code
extension does not run project hooks; use a qualified CLI or Desktop runtime. Nothing is trusted
automatically and I never flip that switch for you — approval stays your manual
