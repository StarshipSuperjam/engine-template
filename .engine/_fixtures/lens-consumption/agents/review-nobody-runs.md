...
`orphaned-review`, which no build stage consumes — no roster named by `review_consumers` in
`.engine/build-protocol.json` carries it. So the review is installed yet no stage runs it against
