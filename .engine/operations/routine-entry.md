...
1. **Confirm the run is non-interactive (it genuinely cannot ask).** The operator configured this routine
   with a non-interactive permission posture — set in their Claude Code permission settings — so it proceeds
   without prompts. This is the operator's setup, not something the engine sets here. If at any point an
   action would require an approval this run cannot give — including that posture being unavailable for the
...
from git and the checklist. The non-interactive permission posture and the per-task worktree toggle are
