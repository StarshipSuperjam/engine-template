...
2. **Confirm isolation from the operator's checkout.** A scheduled run does not isolate into its own
   ...
   halt).
3. **Read git state and the frozen scope-locked build Issue.** The build Issue holds the ordered
   commit-sequence checklist (the durable plan a cold session reads) and the permitted write-scope
   alongside it. If GitHub is unreachable there is no plan to read — exit without proceeding (fail-safe).
   If there is no valid target where one was expected — no build Issue, or a locked-on target that does not
   match — file a durable **misfire Issue** (step 6's helper) and exit; a misfire is operator-visible, never
   a silent exit.
...
6. **Escalate anything that needs a human, because this run cannot ask.** An out-of-scope observation
   ...
   (`.engine/tools/issue_author.py`) so it reads like every engine-authored Issue. A run with no remaining
   eligible scope exits cleanly ("nothing to do").

## Done when

The routine either advanced the build by one chunk — commits added inside the scope-lock, progress
reported, the pull request left open — or exited without proceeding: cleanly when nothing was eligible, or
leaving a durable, operator-visible Issue when it hit a missing or mis-aimed target, a blocker, or a
posture it could not satisfy. In every outcome the protected branch was never merged and the operator's
checkout was never mutated. An interactive Finalize session later integrates, reviews for cohesion,
validates, and submits the pull request for the operator's merge.
