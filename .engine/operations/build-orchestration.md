...
Only after green validation, create `review packet --stage deliverable`. The packet contains the exact raw
intent, approved plan, settled criteria where present, reviewed commit and base, and impact evidence. The
spec-conformance reviewer checks plan-derived success obligations in every Build; settled criteria add a
higher-authority comparison rather than enabling review. The divergence hunter reverse-sweeps the diff
