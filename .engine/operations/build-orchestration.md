...
that pause asks for. It is an offer, not a gate: the bind's own `--operator-decided` consent is the
...
plan-review gate and no waiver for one. Approve, seal and bind each refuse without a recorded operator decision
at that gate, and each checks that the gate before it left its record — a record that the operator was asked, not proof.
