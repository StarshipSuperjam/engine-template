...
The required `engine-ci` check that runs in GitHub after a push has two routes to green. A run for new or changed code — every push, and a pull request opened, pushed, or reopened — runs the full inventory, exactly as before. A run for a metadata-only event on a pull request — a body edit, or a label change such as applying `guardrail-ack` — verifies a receipt an earlier full run left for the IDENTICAL checked-out tree (bound to its content-addressed git tree hash, so it holds only if the content is unchanged) and re-runs only the checks whose verdict can change while the tree is unchanged, skipping the self-test inventory that tree already passed. A reuse run discloses, as the first line of its job summary, that it reused an earlier proof and which run it came from; any doubt — no receipt, an unreadable one, an API failure — resolves to a full run. So applying an acknowledgement label no longer re-spends the whole inventory on a tree already judged.
