...
| Hooks | Engine registration; individual operator trust | **Approve once and re-approve changed hooks through the verified Codex CLI `/hooks` browser and verify an actual event afterward** | Project-folder trust alone is insufficient. Do not promise a Desktop `/hooks` command or settings screen. The September 9 Desktop fixture required individual hook approval and reopening the same task to load its configuration; this is observed recovery, not a per-repository test-harness requirement. If hooks are absent, disclose automation as off, ground manually and require explicit Build authority. See [qualification evidence](codex-validation.md). |
...
| Models and reasoning effort; telemetry, notifications, output, personality | Engine persona models resolve from `.engine/policies/model-bindings.json`: native Codex tiers and overrides live under `providers.codex`, while worker classes retain their existing bindings. Generated reviewer models are pinned from that owner and effort remains absent from reviewer renders; depth selects coverage only. General task defaults and the remaining controls are personal/organizational choices. |
