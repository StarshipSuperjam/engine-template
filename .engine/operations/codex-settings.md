...
| Hooks | Engine registration; operator trust | **Approve once and re-approve after the Engine changes its hooks — via `/hooks` (CLI) or the Hooks screen under Settings (Desktop)** | Codex will not run new or changed project hooks without trust. This manual intervention has outsized value because grounding, the Explore write-gate, and memory capture otherwise turn off. If declined, the assistant must disclose that automation is off, ground manually, and stay read-only until Build is explicit. Both approval paths and the skip-until-trusted behaviour are per OpenAI's hooks page and openai/codex#35306 (2026-09-06). |
