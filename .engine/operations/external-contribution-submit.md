...
6. **Have the change looked over before you submit.** Exact plan and deliverable packets can be constructed in
   the source harness from raw intent, the approved plan, settled criteria, and the commit under review before
   an external pull request exists. The target repository need not contain the coordinator and no draft PR is
   required merely to obtain independent review. For anything beyond a trivial change, run the applicable
   cold review before asking to submit. If it did not run, the prepared request says so plainly rather than
   looking as reviewed as ordinary owned work; **that disclosure is a backstop, not a substitute for review**.
