...
4. **Accepting a plan imports it; it does not start building it.** This changed: accepting a plan used to
   ...
   plans folder (or a `plansDirectory` set in managed or your own settings, never a project's file). On
   Claude the adapter fires whenever you accept a plan in plan mode — a mode you now enter yourself, since
   the Engine sets no permission default for a project (StarshipSuperjam/engine-template#1227). The
