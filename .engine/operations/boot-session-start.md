...
- **A leftover template license — clear it (a reviewed pull request) or keep it (`boot_alarm_ledger.retire`, StarshipSuperjam/engine-template#471).**
  ...
  assistant hands the one-file `LICENSE` removal to the **`trivial` profile** of [Build kickoff](build-kickoff.md) —
  a reviewed pull request the operator merges (a live protected repo's committed license is removed durably
