...
plus the next per-prompt scent — which now fires on every prompt rather than only on a word-match, so the
floor is reached reliably, though it restores the reflex to consult memory rather than the picture itself. The memory reversible-forgetting readout and the modes stance line
