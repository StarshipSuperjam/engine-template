...
1. **Check the GitHub connection, and never dead-end.** Run a quick check (for example `gh repo view`). If it
   fails, say so plainly and name the next action — but a check that fails from inside a sandbox is inconclusive,
   so rerun it outside the sandbox before assuming `gh auth login` — then carry on regardless: capture what the
   operator has already told you as a committed file under `docs/spec/` so nothing they said is lost. The whole
   intake works from committed files; the connection is only needed later, to turn the description into tracked work.
