"""The engine's historically-shipped template-`LICENSE` seeds and the recognizer — ONE permanent home.

Two consumers read this: the **first-run clear** in `instantiator.py` (which retires at first run) and the
...
def _normalize(text: str) -> str:
    """Collapse cosmetic-only variance so a traveled copy that differs solely in line endings (CRLF/CR), blank
    lines, a trailing newline, or a leading byte-order mark still matches, while ANY substantive difference does
    not. A BOM can only fail a match (toward preserve), never spuriously succeed."""
    return " ".join(text.replace("\r\n", "\n").replace("\r", "\n").lstrip("\ufeff").split())


# Stable per-seed id: normalized-seed → a short content hash. Content-derived (not a list index) so it is
# drift-proof and stays the same session-to-session for the same seed (the ledger collapse relies on that),
# while a change to a DIFFERENT recognizable seed yields a different id (so a retired finding re-surfaces on a
# new leak). The map is the single structure the recognizer and the fingerprint both read.
_SEED_ID = {_normalize(s): hashlib.sha256(_normalize(s).encode("utf-8")).hexdigest()[:12] for s in HISTORICAL_SEEDS}


def matched_seed_id(text) -> "str | None":
    """The stable id of the historically-shipped seed `text` matches, or None when it matches none (a product's
    own license, empty, or anchor-edited text). This is the detector's condition fingerprint."""
    if not text:
        return None
    return _SEED_ID.get(_normalize(text))


def recognize(text) -> bool:
    """True iff `text` is one of the engine's OWN historically-shipped template-license seeds (a positive
    self-seed match). Conservative / preserve-on-doubt: empty, None, or anchor-edited text returns False, so a
    product's own license is never matched."""
    return matched_seed_id(text) is not None
