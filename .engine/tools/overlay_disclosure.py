...
def disclosure_registers(changed: list) -> dict:
    ...
    `preserved` is read directly from `PRESERVE_DATA`; `derived` is split back out of the remaining overwrite
    hits. The three sets are disjoint by construction."""
    over_hits = _changed_hits(changed, module_manager.overlay_replace_paths())
    ...
    return {
        ...
        "preserved": _changed_hits(changed, set(module_coherence.PRESERVE_DATA)),
    }
...
def compose_comment(registers: dict, home: str | None) -> str:
    ...
    if derived:
        parts.append(
            "**These engine-internal files are refreshed by the update — your edits to them are rebuilt, not "
            "lost.**\n\n"
            "The engine regenerates them from your own reconciled tree on each update, so an edit here is "
            "reproduced rather than reverted; you do not need to preserve it by hand:\n\n"
