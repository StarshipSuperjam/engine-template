...
inherent slack). It classifies THREE registers off that one source (see `disclosure_registers`): decision-relevant
OVERWRITE risk; routine engine-internal indexes the update REGENERATES from the reconciled tree
(`module_manager.REGENERATED_DERIVED`, retained for classification but silent on their own); and a positive
confirmation for the per-deployment operator DATA the update PRESERVES create-if-absent
(`module_coherence.PRESERVE_DATA` — so a change to bound data is surfaced as kept, never met with silence).
It still never warns about a file the update leaves untouched by being outside the overlay entirely (operator
config and the keyed-merge fences of CLAUDE.md/AGENTS.md/.gitignore).
...
def disclosure_registers(changed: list) -> dict:
    ...
    - `derived` — engine-internal indexes the update REGENERATES from the reconciled tree
      (`module_manager.REGENERATED_DERIVED`): split OUT of the overwrite alarm and retained as a control-only
      classification signal; routine derived churn does not warrant a notice.
...
def compose_comment(registers: dict, home: str | None) -> str:
    """The plain-language, non-blocking disclosure body for decision-relevant overwrite and preserved-data
    changes. Derived paths are deliberately omitted: they remain classified solely to suppress routine churn.
    Peer voice: inform + consequence, route to the durable home (named, when known), never forbid. Every path
    is rendered through `_safe_path`, and each list is capped."""
    ...
    if [bool(overwrite), bool(preserved)].count(True) > 1:
...
def reconcile(client: _Comments, number: int, registers: dict, home: str | None) -> str:
    ...
    duplicate). Returns a plain status word. Posts for overwrite risk or preserved data; derived paths remain
    classified but are control-only, so derived-only transitions retract an existing notice and otherwise stay
    silent."""
    mine = [c for c in client.list_comments(number)
            if COMMENT_MARKER in (c.get("body") or "") and _is_bot(c)]
    if any(registers.get(k) for k in ("overwrite", "preserved")):
