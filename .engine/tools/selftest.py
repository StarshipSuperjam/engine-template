...
def _forward_signal(proc, signum) -> None:
    ...
    that window from swallowing a signal or leaving a launcher nothing can stop (#1188)."""
    if proc is None:
...
def _run_parent(args: argparse.Namespace) -> int:
    ...
    proc = None
    try:
        # The forwarding handlers go in BEFORE the child is spawned (#1188). A signal disposition inherits
