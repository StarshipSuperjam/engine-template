...
Design constraints (so compaction's fold stays legal and tests stay deterministic):
...
# consolidate — that would cycle); a role-less record (a turn-delta, a pre-consolidation record) gets the default.
ROLE_WEIGHTS = {
    "decision": 1.30,
...
def frecency(birth_ts: int, access_ts, now: int) -> float:
    ...
    i.e. compaction can carry one scalar `frecency_snapshot` stamped at `t`, discard the folded-away markers, and
...
def _effective(record, access_ts, now: int):
    ...
    as the recency base. Otherwise the original birth-seeded path. The two agree for the same record-state."""
