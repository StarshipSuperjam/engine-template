...
The scored demotion tiers, the gist roll-up, ledger compaction (the rebuild-and-swap), the stable record id,
and Layer-2 audit-gated erasure are later sub-slices of the memory build plan's step 4.
"""
...
def main(argv: list) -> int:
    ...
    if cmd == "demo":
        return _demo()
    print(f"usage: forget.py [duplicates|demo]\nunknown command {cmd!r}", file=sys.stderr)
    return 2
