...
class LedgerDemotionTests(_Base):
    ...
    def test_back_compat_a_recent_record_with_no_id_or_no_role_still_recalls(self):
        # Case 10: a legacy record (no `id`) and a role-less curated record still score (born hot) and recall; the
...
class SnapshotAwareScoringTests(unittest.TestCase):
    ...
    def test_non_finite_carried_timestamps_never_crash_recall(self):
        # The line-resilience law for the new compaction fields: a corrupt (NaN/inf) carried timestamp on ONE record —
        ...
        # birth `ts` (a latent gap pre-compaction) and a non-finite access timestamp.
        now = 2_000_000_000
        cases = [
