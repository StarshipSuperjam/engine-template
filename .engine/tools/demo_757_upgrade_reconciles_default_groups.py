...
a live upgrade would). Both arms start from the same pristine clone; the only difference is whether the tail's
reconcile can rewrite the drifting selection:
  * POSITIVE (the fix): the release's committed `default-groups` drifts from the deployed set (a single-line
    array the reconciler can rewrite). The tail re-derives from the installed modules and rewrites the line, so
    committed == derived, the structural gate is clean, and the update opens for review. `groups_reconciled` is
    True and the new selection is recorded for the pull-request body.
...
The reconcile is direction-agnostic: here the drift is a group the release under-listed (the deployed set
derives MORE than the release committed), the clean mirror of the field case (a release that listed
`memory-semantic-recall` in `default-groups` on a deployment that declined it). The removal direction and the
pure rewrite/derive helpers are covered by `test_module_manager.TestUpgradeDefaultGroupsReconcile`.
...
def main() -> int:
    ...
    print("update's pull request is born green (never red on uv-group-drift). Same pristine clone, two arms;")
    print("the only difference is whether the tail's reconcile can rewrite the drifting default-groups line.")
    print("=" * 78)
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        reconciled = bool(result.get("groups_reconciled"))
        ...
        print(f"  reconcile reported a change (groups_reconciled): {reconciled}")
        ...
        if not reconciled:
            failures.append("POSITIVE: the tail did not report reconciling default-groups (groups_reconciled False)")
        if not in_sync:
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        print(f"  reconcile failed open (drift survived):       {not result.get('groups_reconciled')}")
