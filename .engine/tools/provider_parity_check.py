#!/usr/bin/env python3
"""Provider parity — the custom/script entry for engine/check/codex-provider-parity (the established design).
...
def findings(tier: str, root: str | None = None) -> list:
    ...
    def miss(kind: str, identity: str, side: str, detail: str):
        ...
        out.append(validate.finding(tier,
                   ...
                   f"{LEDGER_REL} with a reason (the established design).", None))
    ...
    def hooks_with_canary(rel_path: str) -> set:
        ...
        (an empty set equals an empty set). A broken check must never report green (the established design)."""
        try:
