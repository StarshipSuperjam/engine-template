...
# The session never merges the protected branch — that is the operator's own consent act (the established design), and
# an AI performing it would corrupt the very gate the trust model rests on, so the established design's "may hard-fail
...
# operator), NEVER "cannot" (which would dress this fallible local nudge as the wall the established design forbids).
