...
These lock the behaviours a non-engineer cannot read code to verify:
  ...
  - the notice classifies THREE registers off the one overlay source: decision-relevant overwrite risk, routine
    regenerated-derived churn that stays silent, and a positive 'kept' confirmation for preserved per-deployment
    data — so preserved data is surfaced, derived-only churn is silent, and a pure add is never warned about;
...
class TestDisclosureRegisters(unittest.TestCase):
    """The three registers, all read off the ONE overlay source: overwrite risk, regenerated-derived churn
    split OUT of the overwrite set for control-only classification, and preserved-data confirmation (read
    straight from PRESERVE_DATA, which overlay_replace_paths already excludes). Mocked constants so the split
    is asserted on the classifier logic, not on today's live membership."""
...
class TestComment(unittest.TestCase):
    ...
    def test_mixed_overwrite_and_derived_omits_the_derived_section_and_count(self):
        body = od.compose_comment({"overwrite": [".engine/tools/boot.py"],
                                   "derived": [".engine/knowledge/graph.json"]}, HOME)
        self.assertIn(".engine/tools/boot.py", body)
        self.assertNotIn(".engine/knowledge/graph.json", body)
        self.assertNotIn("regenerated by the update", body)
        self.assertNotIn("more than one kind", body)
...
class TestReconcile(unittest.TestCase):
    ...
    def test_derived_only_is_silent_without_a_prior_notice(self):
        client, rec = self._client()
        self.assertEqual(od.reconcile(client, 7, {"derived": [".engine/knowledge/graph.json"]}, HOME), "clean")
        self.assertEqual(rec.counts()["POST"], 0)
        self.assertEqual(rec.counts()["PATCH"], 0)

    def test_derived_only_resolves_a_prior_notice(self):
        prior = {"id": 1, "body": od.COMMENT_MARKER + "\nprior notice", "user": BOT}
        client, rec = self._client([prior])
        self.assertEqual(od.reconcile(client, 7, {"derived": [".engine/knowledge/graph.json"]}, HOME), "retracted")
        self.assertEqual(rec.comments[0]["body"], od._resolved_body())

    def test_preserved_and_derived_posts_only_the_preserved_confirmation(self):
        client, rec = self._client()
        regs = {"preserved": [".engine/memory-backup/pointer.json"],
                "derived": [".engine/knowledge/graph.json"]}
        self.assertEqual(od.reconcile(client, 7, regs, HOME), "posted")
        body = rec.comments[0]["body"]
        self.assertIn(".engine/memory-backup/pointer.json", body)
        self.assertNotIn(".engine/knowledge/graph.json", body)
        self.assertNotIn("more than one kind", body)
