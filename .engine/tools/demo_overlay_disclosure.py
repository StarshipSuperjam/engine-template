...
def main() -> int:
    ...
    # (b3) deployed + a change to a REGENERATED-DERIVED index → a calm 'rebuilt, not lost' line, not the alarm.
    status, fake = _run(
    ...
    assert status == "posted", status
    body = fake.posted[0]
    assert ".engine/knowledge/graph.json" in body, "the calm line must name the derived file"
    assert "rebuilt, not" in body, "the derived register must say the edit is rebuilt"
    assert "won't survive the next update" not in body, "a regenerated file must not draw the overwrite alarm"
