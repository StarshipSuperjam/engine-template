...
Exercises the REAL tool logic (`disclosure_registers` → `compose_comment` → `reconcile`, the deployed gate, the
...
Shows:
  ...
  (b) a deployed repo where the pull request touches only an outside-overlay operator config → silence;
      a configured preserved-data file still draws its positive confirmation;
  (c) routine regenerated-derived churn → silence (no owner notification);
  (d) a rename to an attacker-chosen name → the crafted name is sanitized, never injected;
  (e) an engine-authored update PR → the disclosure is off;
  (f) which repositories it speaks in at all, against REAL throwaway git checkouts: the engine's own home is
...
def _run(title, changed, overwrite):
    """Run the REAL classifier + comment + reconcile against a fresh fake transport. Returns (status, fake)."""
    orig = od.module_manager.overlay_replace_paths
    od.module_manager.overlay_replace_paths = lambda: overwrite
    try:
        registers = od.disclosure_registers(changed)
        fake = _FakeGitHub()
        status = od.reconcile(od._Comments("acme/product", "tok", transport=fake), 7, registers, HOME)
    finally:
        od.module_manager.overlay_replace_paths = orig
    print(f"\n=== {title} ===")
    print(f"  changed files : {[c['filename'] for c in changed]}")
    print(f"  registers     : overwrite={registers['overwrite']} derived={registers['derived']} "
          f"preserved={registers['preserved']}")
...
def main() -> int:
    ...
    # (b) deployed + only an outside-`provides` preserved edit (operator config) → silence: it is in no
    # overlay/register at all, so there is nothing to say.
    status, fake = _run(
        "Deployed repo, a change to only an outside-provides PRESERVED file (operator config)",
        [{"filename": ".engine/operator-overrides.json", "status": "modified"}], OVERWRITE)
    assert status == "clean", status
    assert not fake.posted, "an outside-provides preserved file must never draw a comment"

    # (b2) deployed + a change to PRESERVE_DATA (per-deployment data inside `provides`) → a positive
    # 'your value is kept' confirmation, never silence — a create-if-absent misfire must never be no-signal. #814.
    status, fake = _run(
        "Deployed repo, a change to a PRESERVED per-deployment data file (the backup pointer)",
        [{"filename": ".engine/memory-backup/pointer.json", "status": "modified"}], OVERWRITE)
    assert status == "posted", status
    body = fake.posted[0]
    assert ".engine/memory-backup/pointer.json" in body, "the confirmation must name the preserved file"
    assert "kept" in body.lower(), "the preserved register must confirm the value is kept"
    assert "won't survive the next update" not in body, "a preserved file must never draw the overwrite alarm"

    # (b3) deployed + only a REGENERATED-DERIVED index → silence: routine self-rebuilding churn does not
    # notify the owner, while the classifier still records it as derived.
    status, fake = _run(
        "Deployed repo, a change to a regenerated-derived index (the knowledge graph)",
        [{"filename": ".engine/knowledge/graph.json", "status": "modified"}],
        OVERWRITE | {".engine/knowledge/graph.json"})
    assert status == "clean", status
    assert not fake.posted, "derived-only changes must not post a notice"
    assert not fake.comments, "with no prior notice there is nothing to resolve"
