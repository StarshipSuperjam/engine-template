...
def canonical_catalogue(root: str = validate.ROOT) -> str:
    ...
    out = [
        ...
        "context. Green means ONE of three things happened, and the run's summary says which: a full run happened "
        "here — every hard CI rule clear and every discovered self-test module run against this revision; OR a "
        "receipt from an earlier successful full run of this workflow was verified in its place, bound to the "
        "identical checked-out tree and named with its source run in that run's summary; OR, in a deployed copy "
        "only, the change set was classified project-only and the validator CI suite alone ran (the third route, "
        "below). A reuse run is not a lighter full run: it re-runs only the CI rules whose verdict "
        ...
        "self-test inventory, which a tree already judged cannot change. New or changed Engine code always runs "
        "full; only an event that cannot have changed the tree may reuse, and a receipt is honoured for at most "
        "14 days. In a DEPLOYED copy of the Engine there is a third route, **project-only**: when the change "
        "classifier places every changed path outside everything the Engine owns — outside its corners, its "
        "declared root files and its ownership register, with every doubt resolved to the "
        "Engine's — the run executes the full validator CI suite and NOT the self-test inventory, uploads a "
        "receipt stamped project-only that the reuse route refuses, and states in its summary that its green "
        "attests Engine health only and that no product validation ran. New product code takes that route on "
        "every push of a product-only pull request; the home repository never does. A repository variable "
        "(`ENGINE_CI_PROJECT_ONLY_ARM=off`) switches the route off per deployment. "
