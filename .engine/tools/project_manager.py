...
component also manages task completion and owns the continuous-improvement workflows — so the operator
retitled it on 2026-08-24 to the job it actually does. Organizing work across build phases through the
program object was once part of that job here too; that surface moved to its own address in
program_manager.py (PR 4), so the program verbs live there now and this tool holds the plan half. The
retitle is of the COMPONENT, deliberately not of the data: the schema ids
...
def close_plan_record(library, slug: str, state: str, reason: str, *,
    ...
    retires the plan it replaces through exactly this door rather than through a second
    implementation of it, and this is the ONE plan-record write the program surface may perform. A
    module-scoped pin over program_manager.py enforces that it reaches the plan library no other way.
    A supersession that wrote a closure some other way would be a plan marked closed in the record
    while the projection still advertised it — which is the shape of the loaded gun supersede exists
    to unload.
...
def cmd_program_moved(args) -> int:
    """The old program door. It moved to its own address; this refuses with one pointer and exit 2.

    The whole program verb family now lives in program_manager.py, reached the same way it always was
    — `program <verb>` — only under a different tool name. Nothing here forwards or aliases: a clean
    cut, because the callers are sessions and engine machinery, not a person retyping a command.
    """
    raise ProjectManagerError(
        "the program surface moved to its own address. Run `python tools/program_manager.py program "
        "<verb>` — every verb, argument and behaviour is unchanged; only the tool name is. This door "
        "is closed and forwards nothing.")
