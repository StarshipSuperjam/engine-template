...
def lane_standing(record: dict, view: list) -> dict | None:
    ...
    def classify(member: str) -> tuple:
        # (status, mark, bucket). A member absent from the view is not in this program at all; a
        # missing/unreadable plan cannot be judged — both are unknown, never folded into a count of
        # settled. Superseded and dead branches are settled ends; complete is settled; anything else
        # a child can derive to (active, sealed, awaiting-approval, …) is still in flight.
        if member not in status_of:
            return "not in this program", None, LANE_BUCKET_UNKNOWN
        status = status_of[member]
        if member in superseded_of:
            return status, f" — superseded by `{superseded_of[member]}`", LANE_BUCKET_SETTLED
        if status in _LANE_ATTENTION_STATES:
            return status, None, LANE_BUCKET_UNKNOWN
        if status in DEAD_BRANCH_STATES:
            return status, f" — {status}, marked not hidden", LANE_BUCKET_SETTLED
        if status == "complete":
            return status, None, LANE_BUCKET_SETTLED
        return status, None, LANE_BUCKET_IN_FLIGHT
    ...
    for lane in split["lanes"]:
        ...
        for member in sorted(lane["children"], key=lambda m: (ordinal_of.get(m, 1 << 30), m)):
            laned.add(member)
            status, mark, bucket = classify(member)
            members.append({"plan_id": member, "status": status, "mark": mark, "bucket": bucket})
...
def _render_lanes(record: dict, view: list) -> list:
    ...
    if standing:
        ...
        for row in standing["lane_rows"]:
            rendered = [f"`{m['plan_id']}` ({m['status']}){m['mark'] or ''}" for m in row["members"]]
