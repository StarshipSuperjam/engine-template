...
| BC-30 | recovery behavior | A Build's own execution evidence survives a forced restart of the workstation it runs on. | `build_state_store.DurableBuildStore`; `test_build_state_store`; the 2026-08-25 durable-state change | The snapshot lived in the OS temporary directory by construction, and `StateStore` REFUSED any other path. A Build killed mid-flight lost its approval, review receipts, findings, dispositions and progress; they were reconstructed by hand. | The snapshot is durable and lives beside the sealed plan that bound it, one per plan, written owner-only through the same lock, compare-and-swap and atomic durable write the plan library uses. A session resuming in the same worktree finds it with nothing remembered; a second Build of the same plan displaces the first only through an explicit `state supersede`, which keeps the displaced snapshot. Durability buys survival, not standing: the snapshot still carries no authority, and the library is per-workstation, so it is a restart of THIS machine that is survived, not a move to another. |
| BC-31 | mechanical fact | The repair loop is bounded by what its rounds SPEND, and both bounds are guidance stops, never walls. | `cmd_repair_assess`; `test_build_coordinator.TestValidationRepairAndStatus`; the 2026-08-25 spend-counting amendment | Nothing bounded the loop until StarshipSuperjam/engine-template#1049 added a flat two-round stop, and that control was never recorded in this table — the row corrects an omission before it describes a change. The flat count then measured the wrong thing: a `none` judgment and a full cold panel cost the same, so the honest bound sat below what a converging repair needs, while an unattended session could still spend indefinitely on rounds that each looked small. | A round dispatching two or more cold lenses is COUNTED; three counted rounds is the budget, six rounds of any kind the absolute ceiling. Either stop refuses until `--guidance` records the operator's answer, which is published in the PR body with the rounds record. What a repair touched is evidence, the default lens roster, and disclosure — never the budget key and never a judgment selector, so BC-16 and BC-17 stand unchanged: round accounting bounds HOW MANY rounds run, never which lenses run or how deep. Counting is by ROSTER SIZE, not by model or effort, and the cheap check's minimal model is a stated convention rather than a mechanism — so the honest worst case these bounds allow is three full panels plus three further single-lens rounds at any effort, and a reader is owed that number rather than an impression of cheapness. |
>>>>>>> a830ab6 (Reconcile onto the sunset of build-state.v1)
