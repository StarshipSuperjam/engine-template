...
_Amended 2026-08-25 for the Review section's genre (BC-31 ADDED). This one is a REDUCTION in what the merge surface discloses, and it is recorded as a decision rather than as a defect fix, because that is what it is. The operator directed a study before any design: read the hundreds of pull requests that predate the Build coordinator, judge what the body is good at carrying, and say how much it had been overburdened. The measurement is in the row above. The finding was that the body has one strength — a consequence-weighted account written for the merge decision — and that the coordinator had spent it on transcription, recording every detail of every step for every conceivable future reader until the surface could not be published. What a reader LOSES: the full text of a settled finding no longer reaches a merged body, and since the plan library is gitignored and machine-local, that text becomes unreadable from a clone. That cost is real, it is permanent per merged build, and it was accepted deliberately on the operator's ruling that plan-gate detail has served its purpose by the time the code exists. What is NOT weakened, and is what the guarantee always actually rested on: every disagreement still renders verbatim through its own renderer, everything still open renders in full, and the operator's own read at merge is unchanged. BC-16 and BC-17 stand — what a finding was informs and discloses; it never selects a judgment or a depth._

_Amended 2026-08-25 for spend-counted repair rounds. One assertion is ADDED (BC-32), and it does two things
at once, which is stated plainly rather than blurred. First it RECORDS a control this table never carried:
the repair-round stop has existed since StarshipSuperjam/engine-template#1049 and was never written down
here, so a reader of this record could not have known the loop was bounded at all. Second, that control is
WEAKENED — disclosed as a weakening, in the form the 2026-08-24 amendment established. The stop moves from
two rounds of any judgment to three rounds that dispatch a review panel, with a new absolute ceiling of six
rounds of any kind behind it. What a reader loses: a Build may now run three cold panels before it must stop
for the operator, where two used to be the wall, and cheap rounds no longer count toward that wall at all.
What replaces it: an absolute ceiling that the old control did not have, so the unbounded-cheap-rounds path
the flat count left open is closed; rounds that are narrower by construction, because a scoped round now
defaults to the lenses that raised blockers and its packet names the repair as the review's subject; and a
rounds record published at the merge gate, so what the loop actually spent is in front of the operator whose
approval it spends. The weakening is recorded even though the moved number is not a hard hold, because a
control this table does not name cannot be checked by anyone reading it. BC-16 and BC-17 are untouched: what
a round touched informs and discloses, and never selects a judgment or a depth._
...
| BC-31 | disclosure behavior | The pull request's Review section is an ACCOUNT of what the operator must still weigh, not a transcript of every finding raised. | `partition_findings`; `_plan_finding_lines`; `build_coordinator_contract.compose`; the 2026-08-25 corpus study of 547 merged pull requests | The composer transcribed both finding ledgers row by row. Measured: 520 hand-written merges carried this same template at a 9.6KB median with Review a ~2KB narrative (21% of the body); the four coordinator-composed merges ran at a 43.5KB median with Review at 28KB (64%), and the fifth could not be published at all — StarshipSuperjam/engine-template#1080 refused at 64,313 bytes against GitHub's 60,000-byte budget, with the author's own account cut three times to try to fit. The template never changed; the GENRE of one section did, and the only part that could give was the judgment nobody else could write. | One shared predicate splits both ledgers exhaustively and exactly once. Rendered in full: still-blocking, escalated, rejected, or partially-accepted — plus, Build-side only, accepted-tracked, because deferred work is live risk. It keys on the UNION of flag and disposition because the plan record constrains neither against the other. Counted, never transcribed: the settled remainder, as `len()` projections of the very lists rendered. Untouched: the disagreement lines, and the claim's 1:1 operator-safe summary per finding, so the handoff keeps full fidelity. Two things are said rather than implied — an unreadable plan record renders as unknown, never as a zero count, and the digest states that the full plan text lives only in the workstation-local library, which is gitignored and never published. |
| BC-32 | mechanical fact | The repair loop is bounded by what its rounds SPEND, and both bounds are guidance stops, never walls. | `cmd_repair_assess`; `test_build_coordinator.TestValidationRepairAndStatus`; the 2026-08-25 spend-counting amendment | Nothing bounded the loop until StarshipSuperjam/engine-template#1049 added a flat two-round stop, and that control was never recorded in this table — the row corrects an omission before it describes a change. The flat count then measured the wrong thing: a `none` judgment and a full cold panel cost the same, so the honest bound sat below what a converging repair needs, while an unattended session could still spend indefinitely on rounds that each looked small. | A round dispatching two or more cold lenses is COUNTED; three counted rounds is the budget, six rounds of any kind the absolute ceiling. Either stop refuses until `--guidance` records the operator's answer, which is published in the PR body with the rounds record. What a repair touched is evidence, the default lens roster, and disclosure — never the budget key and never a judgment selector, so BC-16 and BC-17 stand unchanged: round accounting bounds HOW MANY rounds run, never which lenses run or how deep. Counting is by ROSTER SIZE, not by model or effort, and the cheap check's minimal model is a stated convention rather than a mechanism — so the honest worst case these bounds allow is three full panels plus three further single-lens rounds at any effort, and a reader is owed that number rather than an impression of cheapness. |
| BC-33 | recovery behavior | A session verifies that it matches the Build it is about to change, before every mutating verb, whether or not it knows its context was compacted. | `build_coordinator.verify_resume`; `test_build_coordinator.TestUnconditionalResumeVerification`; the 2026-08-26 context-control change | The snapshot recorded `bound_head` at bind and at adopt and NOTHING ever read it — a provenance field with no reader, verifiable at 5285a29. Nothing compared the running session's worktree against the snapshot's when `--state` named one explicitly, and nothing noticed a HEAD that had left the Build's line. A session resuming from a compaction summary could therefore mutate a Build it had the wrong idea about, and the coordinator would record the result as ordinary evidence. | Every verb absent from a small read-only set re-reads the snapshot and refuses, by name, when the recorded worktree is not the one the session stands in or when the bound commit is no longer an ancestor of HEAD. It is UNCONDITIONAL: it never asks whether a compaction happened, because that question can only be answered by a fail-open hook, and a guarantee resting on a best-effort signal is the signal's reliability wearing a guarantee's name. It READS only — a write here would bump the revision under the compare-and-swap guard the verb is about to hold, so the check protecting the Build would be the thing that wedged it. A snapshot bound before these fields existed makes no new demands. |
...
| The session does not match the Build it is mutating (wrong worktree, or a HEAD off the Build's line) | A session working from a compaction summary writes evidence into a Build it has the wrong idea about — the wrong Build's snapshot, or the right one after the worktree was reset or switched away. Demonstrated by absence, and checkable: `bound_head` was recorded at bind from the day the field existed and read by nothing, and the worktree comparison existed only on the lookup path, so an explicit `--state` bypassed it entirely. The hold is unconditional rather than compaction-triggered, because the only signal that a compaction occurred is a fail-open hook, and a hold that defers to it is not a hold. Read-only verbs are exempt: refusing `status` on a moved worktree would hide the diagnosis the operator opened it to get. |
