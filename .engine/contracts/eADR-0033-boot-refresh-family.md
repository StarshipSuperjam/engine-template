...
It also locks the byte-fit as **governed, not emergency**: every component that grows carries a named budget and a set-aside rank, so trimming becomes a stated policy rather than a silent truncation, and a margin canary guarantees the never-shed core keeps headroom so one added line cannot quietly tip it out. The growth-vector table (dials in the `briefing-budget` policy):
...
| Status dashboard | project state, alarm count | `dashboard_chars_max` (growth alarm) | 4 (last set aside) |
| Never-shed core margin | all never-shed content | `margin_floor_chars` (hard code min) | never set aside |
